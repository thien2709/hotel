...
const BookingForm = () => {
    ...
    const calculatePayment = () => {
        ...
        const price = roomPrice ? roomPrice * diffInDays : 0
        return price
    }
    ...
    const isGuestCountValid = () => {
        ...
        const totalCount = isNaN(childrenCount) ? adultCount : adultCount + childrenCount
        return totalCount >= 1 && adultCount >= 1
    }

    const isCheckOutDateValid = () => {
        if (!moment(booking.checkOutDate).isSameOrAfter(moment(booking.checkInDate))) {
            setErrorMessage('Check-out date must com before check-in date')
            return false
        } else {
            setErrorMessage('')
            return true
