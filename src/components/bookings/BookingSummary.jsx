...
const BookingSummary = ({ booking, payment, isFormValid, onConfirm }) => {
    ...
    const numOfChildren = isNaN(parseInt(booking.numOfChildren)) ? 0 : parseInt(booking.numOfChildren)
    ...
    return (
        <div className='card card-body mt-5'>
            ...
            <p>Check-Out Date: <strong>{moment(booking.checkOutDate).format('DD MM YYYY')}</strong></p>
            <p>Number of Days: <strong>{numOfDays}</strong></p>
            <h5>Number of Guests</h5>
            <strong>Adult{booking.numOfAdults > 1 ? 's' : ''}: {booking.numOfAdults}</strong>
            <strong> Children: {numOfChildren}</strong>
            {payment > 0 ? (
                <>
